...
use crate::generated::{
    Function as FunctionDefinition, Header_Statement, Newline_Statement, SourceFile as Module,
    Statement,
};
...
impl Header_Statement {
    fn build_document_symbols(
        ...
        match self {
            Header_Statement::Statement(Statement::Function(f)) => {
...
impl Newline_Statement {
    fn build_document_symbols(
        ...
        let statement = if let Newline_Statement::Statement(statement) = self {
            statement
        } else {
            return Ok(());
        };
        match statement {
            Statement::Function(function) => function.build_document_symbols(doc, ast, builder),
            _ => Ok(()),
        }
    }
}
