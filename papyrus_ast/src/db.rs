...
configure_parsers!(
    PARSERS,
    "psc" => {
        language: tree_sitter_papyrus_sse::LANGUAGE,
        ast_root: SourceFile
    }
);
...
pub fn create_json_db(source_code: &'static [&str]) -> impl BaseDatabase {
    ...
    source_code.iter().enumerate().for_each(|(i, source_code)| {
        let url = Url::parse(&format!("file:///test{i}.psc")).expect("Failed to parse URL");

        let file = File::from_string()
            .db(&db)
            .source(source_code.to_string())
            .url(&url)
            .parsers(PARSERS.get("psc").expect("papyrus parser not found"))
