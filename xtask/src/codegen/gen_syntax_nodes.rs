...
pub(crate) fn generate_nodes(kinds: KindsSrc, grammar: &CstSrc) -> Result<String> {
    ...
    pub(crate) fn token_kind(&self) -> Option<proc_macro2::TokenStream> {
        match self {
            Field::Token(token) => {
                ...
                let name = match name.as_bytes().first() {
                    Some(b'@' | b'#') => {
                        format_ident!("{}", name[1..].to_owned().trim()).into_token_stream()
                    } // #ident -> ident
                    _ => token,
                };
                Some(quote! { S![#name] })
        ...
        match self {
            Field::Token(name) => {
                let name = match name.as_str() {
                    ...
                    "." => "dot",
                    "..." => "dotdotdot",
                    _ => match name.as_bytes().first() {
                        Some(b'@' | b'#') => &name[1..], // #ident -> ident
                        _ => name,
                    },
                };
